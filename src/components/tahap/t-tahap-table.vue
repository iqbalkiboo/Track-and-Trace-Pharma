...
import { Add, EllipsisVertical, CheckmarkCircleOutline, CloseCircleOutline } from "@vicons/ionicons5"
...
const options = ref<SelectOption[]>([
  ...
  { label: "Belum Diperiksa", value: "uncheck" },
...
const levelData = computed(() => props.value.list.map((e: any) => e.code))
const dataIsValid = ref(false)

async function handleClickScan() {
  dataIsValid.value = levelData.value.includes(modalScan.value.code)
  if (!dataIsValid.value)
    tMessage.error('Kode yang di-scan tidak sesuai dengan list')
  
  await getDetailByCode(modalScan.value.code, levelParams.value.level)
  if (scanDataRes.value.status === 'void') {
    modalConfirm.value.show = false
    modalLevel.value.show = false
    tMessage.error('Status PK void')
  } else {
    handleShowLevel()
    selectedData.value = []
...
const generated = ref([])
const totalGenerated = computed(() => generated.value.reduce((a: number, b: any) => a + b.qty, 0))
const percentage = computed(() => {
  let percentageValue = progressValue.value
  if (['pass', 'void'].includes(modalConfirm.value.type)) {
    percentageValue /= totalGenerated.value
  } else if (['reception'].includes(modalConfirm.value.type)) {
    percentageValue /= totalBulk.value
  }
  return Math.floor(percentageValue ? percentageValue * 100 : 0)
})

const selectedGeneratedIds = computed(() => selectedData.value.map((e: any) => e.generated_id))

function handleCheckProgressPass() {
  progressValue.value = 0
  const id = route.params.id
  generated.value = []
  checkingModel.getDetailChecking(id).then((res) => {
    generated.value = res.data.serialize.reduce((a: any, b: any) => {
      for (let i = 0; i < b.generates.length; i++) {
        if (selectedGeneratedIds.value.includes(b.generates[i].id))
          a.push(b.generates[i])
      }
      return a
    }, [])
    checkProgressPass()
...
async function checkProgressPass() {
  authStore.resetTimeout()
  checkingModel.getCheckingLevel(levelParams.value)
    .then((res) => {
      const list = res.data.list.reduce((a: any, b: any) => {
        if (selectedGeneratedIds.value.includes(b.generated_id)) {
          a.push(b)
        }
        return a
      }, [])

      progressValue.value = list.reduce((a: number, b: any) => {
        if (modalConfirm.value.type === 'pass')
          return a + (b.serialize.total_pass || 0)
        if (modalConfirm.value.type === 'void')
          return a + (b.serialize.total_check_void || 0)
        return a
      }, 0)
      
      progressTimeout.value = setTimeout(checkProgressPass, 10000)
    })
    .catch((err: any) => {
      isLoading.value = false
      isBtnLoading.value = false
...
async function handleConfirmReception() {
  ...
  if (modalReception.value.isBulk) {
    ...
    if (dataRes.value.list.length > 0) {
      ...
      totalBulk.value = arrTemp.length
      if (arrTemp.length > 0) {
        for (const element of arrTemp) {
          ...
          await checkingModel.receptionChecking(id, formData).then(() => {
            ++progressValue.value
            if (progressValue.value === arrTemp.length) {
              isBtnLoading.value = false
              handleCloseConfirm()
...
function showSuccessFeedback(type = 'success') {
  checkGroupValue.value = []
  tFeedbackRef.value.type = type
  tFeedbackRef.value.hideIcon = false
  tFeedbackRef.value.iconColor = type === 'danger' ? '#D03050' : '#18a058'
  tFeedbackRef.value.icon = type === 'danger' ? CloseCircleOutline : CheckmarkCircleOutline
...
function checkAllPass() {
  ...
  if (Object.keys(props.value).length > 0) {
    ...
    if (arrTemp.includes('packing_done')) {
      isPass = false
    } else isPass = !(arrTemp.includes('void') && !arrTemp.includes('pass'));
  }
  return isPass
}
</script>
...
<template>
  <n-space v-if="dataRes?.list?.length !== 0" vertical :size="16">
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {{ parent.serialize.total_check_all_void || 0  }} Void
                  </n-button>
                  <n-button
                    v-if="parent.status === 'packing_done' && 'pre_release'"
                ...
                <n-space v-else>
                  ...
                  >
                    {{  parent?.status === 'packing_done' ? 'Belum Diperiksa' : parent?.status === 'void' ? 'Void' : parent?.status === 'pass' ? 'Pass' : 'Tersimpan' }}
                  </div>
                      ...
                      <n-table style="margin-bottom: 10px" :single-line="false">
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td v-if="child.serialize.level === 2" style="width: 4%">
                              <n-button
                                :loading="isBtnLoading"
                                text
                                type="primary"
                        ...
                        <div class="table-tnt" v-if="child.id === detailId">
                          <n-scrollbar x-scrollable>
                            <n-table style="margin-bottom: 10px" :single-line="false">
                              ...
                              <tbody>
                                <tr v-for="valueChild, index in detailChild" :key="index">
                                  <td>{{ index + 1}}</td>
                                  <td>
                                    <div
                                      style="cursor: pointer"
  ...
  <n-space v-else vertical :size="16">
    <t-empty-state words="Tidak ada PK yang siap dicek"></t-empty-state>
  </n-space>
  ...
  <t-tahap-level-modal
    ...
    v-model:is-valid="dataIsValid"
    :parent-data="scanDataRes.parent"
  ...
  >
    <template v-if="isBtnLoading">
      <n-space
        v-if="['pass', 'void'].includes(modalConfirm.type)"
        class="mb-5"
        vertical
      >
        <n-progress
          ...
          style="width: 400px"
        />
        <div class="text-xs text-center">({{progressValue}}/{{totalGenerated}})</div>
      </n-space>
      <n-space
        v-if="modalReception.isBulk && ['reception'].includes(modalConfirm.type)"
        class="mb-5"
        vertical
      >
        <n-progress
          ...
          style="width: 400px"
        />
        <div class="text-xs text-center">({{progressValue}}/{{totalBulk}})</div>
      </n-space>
    </template>
