    ...
    <n-grid cols="3" item-responsive responsive="screen">
      ...
      <n-gi span="3">
        <n-space :size="24">
          <n-p style="margin: 0">
            <n-text strong>{{ 
            props.data?.status === 'Diambil' ? 'Pickup ' : 
            props.data?.status === 'Dikirim' ? 'Send ' : '' }} By : {{ props.data?.created_by?.is_superadmin ? props.data?.created_by?.superadmin?.name : props.data?.created_by?.user?.name 
            }}</n-text>
          </n-p>
        </n-space>
      </n-gi>
