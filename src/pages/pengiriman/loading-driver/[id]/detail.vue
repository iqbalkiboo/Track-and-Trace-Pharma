...
const detaiDriverDataRes = ref<any>([])
...
async function getDetailManifest() {
  ...
  await loadingModel
    .getDetailManifest(id)
    .then(async (res) => {
      detailDataRes.value = res.data
      detaiDriverDataRes.value = detailDataRes.value.driver_info.drivers?.map(
  ...
  <div class="body-tnt">
    ...
    <template v-else>
      <n-space
        ...
        :size="24"
      >
        <n-row style="display: flex; justify-content: space-between">
          <n-col :span="10">
            <n-space
              vertical
              :size="0"
            >
              ...
              <n-space vertical>
                ...
                <n-text
                  ...
                  {{
                    moment(detailDataRes.target_delivery_date).format(
                      "yyyy-MM-DD"
                    )
                  }}</n-text
                >
        ...
        >
          <n-card
            v-for="(driver, key) in detaiDriverDataRes"
            :key="key"
          ...
          >
            <n-grid cols="3">
              <n-gi style="font-size: 13px">
                ...
                <div>
                  Plat Nomor Kendaraan : {{ driver.vehicle?.plate_number }}
                </div>
              </n-gi>
              <n-gi style="font-size: 13px; text-align: center">
              ...
              <n-gi>
                <n-space justify="end">
                  <n-button
                    text
                    v-if="driver.packings.length"
                    type="primary"
                    @click="driver.active = !driver.active"
                    >{{ driver.active ? "Tutup" : "Detail" }}</n-button
                  >
                </n-space>
              </n-gi>
            </n-grid>
            ...
            <n-collapse-transition
              :show="driver.active && driver.packings.length"
            >
              <n-space
                      ...
                      >
                        ...
                        <n-tbody>
                          ...
                          >
                            ...
                            <n-td>{{
                              serialization.serialization?.product_name
                            }}</n-td>
        ...
        <template v-for="(log, iLog) in logDataRes.list">
          <n-card
          ...
          >
            <n-space vertical>
              <n-text
                style="font-size: 14px; font-weight: 500; margin-bottom: 0px"
                >Penerima</n-text
              >
              <n-row>
                <n-col :span="12">
                  ...
                  <div style="margin-bottom: 0.5rem">
                    ...
                    >
                      ...
                      <n-text>{{
                        log.attributes?.receiver?.department
                      }}</n-text>
                    </n-space>
                  </div>
                  <div style="margin-bottom: 0.5rem">
                ...
                <n-col :span="12">
                  <n-text style="font-weight: 500; margin-bottom: 0px"
                    >Tanggal</n-text
                  >
                  <div style="margin-bottom: 0.5rem">
                    <n-text>{{
    ...
    <div class="bottom-action-button">
      <div class="action-item">
        <router-link
          :to="{ name: 'pengiriman-loading-driver' }"
