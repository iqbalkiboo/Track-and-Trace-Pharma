<script setup lang="ts">
  ...
  function addDataItem() {
    dataTablePrimer.value.push({
      id_serial: '001928171' + dataTablePrimer.value.length,
      type: 'Tertiery',
      item: 'Covid-19 Vaccine Astrazeneca',
      qty_primer: 50,
    })
  ...
  function handleSelect(key: string) {
    if (key == 'delete') {
      dataTablePrimer.value.splice(0, 1)
    }
  }

  function saveKemasan() {
    router.push({ name: 'pengiriman-add-manifest' })
  }
</script>
  ...
  <n-space vertical style="padding: 1rem">
    <n-row :cols="12">
      <n-space vertical :size="0">
        <n-space>
          <n-h1 style="margin-bottom: 0"> Tambahkan ke Packing </n-h1>
        </n-space>
    ...
    >
      ...
      <n-tbody>
        <n-tr v-for="(item, key) in dataTablePrimer" :key="key">
          <n-td>{{ key + 1 }}</n-td>
    ...
    <n-table
      ...
      :bordered="false"
      :single-line="true"
      v-if="dataTableSekunder.length > 0"
    >
      <n-thead>
        <n-tr style="border-radius: 20px">
          <n-th style="background-color: #f2994a; color: white">No</n-th>
          ...
          <n-th style="background-color: #f2994a; color: white">Type</n-th>
          <n-th style="background-color: #f2994a; color: white">Item</n-th>
          <n-th style="background-color: #f2994a; color: white">Qty Sekunder</n-th>
          <n-th style="background-color: #f2994a; color: white">Aksi</n-th>
        </n-tr>
      </n-thead>

      <n-tbody>
        <n-tr v-for="(item, key) in dataTableSekunder" :key="key">
          <n-td>{{ key + 1 }}</n-td>
          <n-td>{{ item.id_serial }}</n-td>
          <n-td>{{ item.type }}</n-td>
          <n-td>{{ item.item }}</n-td>
    ...
    <n-table
      ...
      :bordered="false"
      :single-line="true"
      v-if="dataTableTersier.length > 0"
    >
      <n-thead>
        <n-tr style="border-radius: 20px">
          <n-th style="background-color: #f2994a; color: white">No</n-th>
          ...
          <n-th style="background-color: #f2994a; color: white">Type</n-th>
          <n-th style="background-color: #f2994a; color: white">Item</n-th>
          <n-th style="background-color: #f2994a; color: white">Qty Tersier</n-th>
          <n-th style="background-color: #f2994a; color: white">Aksi</n-th>
        </n-tr>
      </n-thead>

      <n-tbody>
        <n-tr v-for="(item, key) in dataTableTersier" :key="key">
          <n-td>{{ key + 1 }}</n-td>
          <n-td>{{ item.id_serial }}</n-td>
          <n-td>{{ item.type }}</n-td>
          <n-td>{{ item.item }}</n-td>
