...
async function getData() {
  ...
  await serializeModelBundling.getDetailSerialize(id).then((res) => {
...
function getDetailSerializeProduct() {
  const id = route.params.id as string
  serializeModelBundling.getDetailSerializeProduct(id).then((res) => {
...
function updateSerialize() {
  ...
  serializeModelBundling.updateSerialize(id, formData).then(() => {
      serializeModelBundling.getGenerateSerialize(id, paramasQty, formValue.value.lot).then(() => {
  ...
  <div class="body-tnt">
              ...
              >
                <n-grid-item span="4 m:1">
                  <n-form-item label="Tanggal Manufaktur" path="manufacture_date">
                    <n-date-picker
                      :disabled="detailDataRes.date?.generate_at !== null"
                ...
                <n-grid-item span="4 m:1" v-for="item, i in formValue.serialize" :key="i">
                  ...
                  >
                    <n-space vertical style="width: 100%">
                      <n-input-number
                        :disabled="i === 0 || detailDataRes.date?.generate_at !== null"
    ...
    <div class="bottom-action-button">
      <div class="action-item">
        ...
        <template v-if="['draft'].includes(detailDataRes.status?.global.value)">
          <n-button
  ...
  >
    ...
    <n-space justify="center">
      ...
      <template v-if="tFeedbackRef.type == 'success'">
        ...
        <router-link
          :to="{ name: 'produksi-bundling-id-print' }"
          class="text-decoration-none"
        >
