  ...
  if (value.form !== 0 && value.to !== 0) {
    ...
    await serializeModel
      ...
      .then((res) => {
        ...
        isQrLoading.value = false
        isBtnLoading.value = false
        serialArr.value = res.data.list.map((v: any) => v.code)
      })
      .catch((err: any) => {
        isLoading.value = false
        isQrLoading.value = false
...
function handleShowConfirmPrint() {
        ...
        <n-grid cols="6" item-responsive responsive="screen" x-gap="26" y-gap="16">
          <n-grid-item span="3 s:2 m:1" v-for="(serialize, i) in detailDataRes.serialize" :key="i">
            <n-button :loading="isBtnLoading" block color="#219653"
              ...
              {{ (serialize as any).name }} ({{ (serialize as any).generated > 0 ? (serialize as any).generated - (serialize as any).total_printed : 0 }})
            </n-button>
          </n-grid-item>
        </n-grid>
      </div>
                              ...
                              <p v-if="
                                ...
                                (detailProductDataRes.serialize?.print
                                  ...
                                  detailProductDataRes.serialize?.print
                                    ...
                                    .het === true)
                              " class="qr-text">
                                {{ $filters.toRupiah(detailDataRes.product?.highest_retail_price) }}
                              </p>
                            </div>
                            <div class="qr-label">
                              <p v-if="
