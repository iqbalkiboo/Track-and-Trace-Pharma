...
onMounted(() => {
  setIsGranted()
  if (filters.isGranted(constants.accessSettings.settingUser + ".list")) {
    getData()
    getWarehouseType()
    getUserType()
  } else {
...
function setIsGranted() {
  if (authData.xxtype === "5up3r4dm1n" || filters.isGranted(constants.accessSettings.settingUser + ".reset")) {
  ...
  if (filters.isGranted(constants.accessSettings.settingUser + ".delete")) {
    actionOptions.value.push({
      label: "Hapus Data",
      key: "delete",
...
async function getDataUserType(status: any) {
  if (status) {
    params.value.user_type = status
  }
  getData()
}
...
const optionsUserType = ref<any>([])
async function getUserType() {
  isLoading.value = true
  accessRightsModel.getRoleType({ limit: 100 }).then((res: any) => {
    res.data.list.forEach((item: any) => {
      optionsUserType.value.push({
        key: item.key,
        value: item.key,
        label: item.name,
      })
...
function handleSelectDropdown(key: string | number, id: any, name: any) {
  switch (key) {
    case "edit":
      router.push({
        path: `/setting/master-user/update/${id}`,
      })
      break
    case "delete":
...
function showSuccessFeedback() {
  ...
  tFeedbackRef.value.subtitle = "Anda berhasil menghapus data user"
...
<template>
  <div class="body-tnt">
    <n-h2 style="margin-bottom: 4px">Master User</n-h2>
    <n-breadcrumb>
      <n-breadcrumb-item>Master User</n-breadcrumb-item>
    </n-breadcrumb>

    <div class="card-tnt">
      <n-space
        v-if="$filters.isGranted(constants.accessSettings.settingUser + '.create')"
      >
        <router-link
          :to="'/setting/master-user/create/1'"
      ...
      <template
        v-if="$filters.isGranted(constants.accessSettings.settingUser + '.list')"
      >
        <n-row
          gutter="16"
        ...
        >
          ...
          <n-col :span="6">
            <n-select
              ...
              style="max-width: 100%"
              @update:value="getDataUserType($event)"
            />
          </n-col>
          <n-col :span="6">
            <n-select
