...
onMounted(() => {
  setIsGranted()
  if (filters.isGranted(constants.accessSettings.settingPerusahaan + ".list")) {
    getData()
  } else {
    return router.push('/')
  }
...
function setIsGranted() {
  ...
  if (filters.isGranted(constants.accessSettings.settingPerusahaan + ".delete")) {
    actionOptions.value.push({
      label: "Hapus Data",
      key: "delete",
...
function handleSelectDropdown(key: string | number, id: any, name: any) {
  switch (key) {
    case "edit":
      router.push({
        path: `/setting/master-company/update/${id}`,
      })
      break
    case "delete":
...
function showSuccessFeedback() {
  ...
  tFeedbackRef.value.subtitle = "Anda berhasil menghapus data company"
...
function deleteCompany() {
  ...
  masterCompanyModel
    .deleteCompany(selectedValue.value.id)
    .then(() => {
      tFeedbackRef.value.show = false
      setTimeout(() => {
        showSuccessFeedback()
...
<template>
  <div class="body-tnt">
    <n-h2 style="margin-bottom: 4px">Perusahaan</n-h2>
    <n-breadcrumb>
      <n-breadcrumb-item>perusahaan</n-breadcrumb-item>
    </n-breadcrumb>
    ...
    <div class="card-tnt">
      ...
      >
        <router-link
          :to="{ path: '/setting/master-company/create/1' }"
      ...
      <template
        v-if="$filters.isGranted(constants.accessSettings.settingPerusahaan + '.list')"
      >
        <n-row
          gutter="16"
              ...
              >
                ...
                <n-tbody>
                  ...
                  >
                    ...
                    <n-td data-label="Nama Perusahaan">{{ list.name }}</n-td>
                    <n-td data-label="Erp Code">{{ list.erp_code }}</n-td>
                    <n-td
                      data-label="Aksi"
                      style="text-align: center"
                    >
