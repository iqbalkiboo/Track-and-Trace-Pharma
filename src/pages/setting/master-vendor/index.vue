...
onMounted(() => {
  setIsGranted()
  if (filters.isGranted(constants.accessSettings.settingVendor + ".list")) {
    getData()
  } else {
    return router.push('/')
  }
...
function setIsGranted() {
  ...
  if (filters.isGranted(constants.accessSettings.settingVendor + ".delete")) {
    actionOptions.value.push({
      label: "Hapus Data",
      key: "delete",
...
function handleSelectDropdown(key: string | number, id: any, name: any) {
  switch (key) {
    case "edit":
      router.push({
        path: `/setting/master-vendor/update/${id}`,
      })
      break
    case "delete":
...
async function deleteSoftVendor() {
  isBtnLoading.value = true
  await masterVendorModel
    .deleteSoftVendor(selectedValue.value.id)
    .then(() => {
      tFeedbackRef.value.show = false
      setTimeout(() => {
        showSuccessFeedback()
...
function showSuccessFeedback() {
  ...
  tFeedbackRef.value.subtitle = "Anda berhasil menghapus data vendor"
...
<template>
  <div class="body-tnt">
    <n-h2 style="margin-bottom: 4px">Master Vendor</n-h2>
    <n-breadcrumb>
      <n-breadcrumb-item>Master Vendor</n-breadcrumb-item>
    </n-breadcrumb>
    ...
    <div class="card-tnt">
      <n-space>
        ...
        >
          <router-link
            :to="{ path: '/setting/master-vendor/create/1' }"
        ...
        <template
          v-if="
              $filters.isGranted(constants.accessSettings.settingVendor + '.import')
            "
        >
          <n-button
            color="#219653"
      ...
      <template
        v-if="$filters.isGranted(constants.accessSettings.settingVendor + '.list')"
      >
        <n-row
          gutter="16"
        ...
        >
          <n-col :span="6">
            <n-input
              ...
              @input="searchData"
              type="text"
              placeholder="Cari Nama Vendor atau Kode Vendor"
            />
          </n-col>
        </n-row>
        <div class="table-tnt">
                ...
                <tbody>
                  <template v-if="dataRes?.list?.length > 0">
                    ...
                    >
                      ...
                      <td>{{ list.pic.name }}</td>
                      <td>{{ list.pic.phone }}</td>
                      <td>{{ list.pic.email }}</td>
                      <td>
                        <n-image
                          width="30"
                          :src="list.npwp?.url"
    ...
    <n-space justify="center">
      ...
      <template v-if="tFeedbackRef.type == 'delete'">
        ...
        <n-button
          ...
          @click.prevent="deleteSoftVendor"
        >
          Iya
        </n-button>
      </template>
