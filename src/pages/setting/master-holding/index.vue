...
onMounted(() => {
  setIsGranted()
  if (filters.isGranted(constants.accessSettings.settingHolding + ".list")) {
    getData()
  } else {
    return router.push('/')
  }
...
function setIsGranted() {
  ...
  if (filters.isGranted(constants.accessSettings.settingHolding + ".delete")) {
    actionOptions.value.push({
      label: "Hapus Data",
      key: "delete",
...
function handleSelectDropdown(key: string | number, id: any, name: any) {
  switch (key) {
    case "edit":
      router.push({
        path: `/setting/master-holding/update/${id}`,
      })
      break
    case "delete":
...
function showSuccessFeedback() {
  ...
  tFeedbackRef.value.subtitle = "Anda berhasil menghapus data holding"
...
function deleteRole() {
  ...
  masterHoldingModel
    .deleteHolding(selectedValue.value.id)
    .then(() => {
      tFeedbackRef.value.show = false
      setTimeout(() => {
        showSuccessFeedback()
...
<template>
  <div class="body-tnt">
    <n-h2 style="margin-bottom: 4px">holding</n-h2>
    <n-breadcrumb>
      <n-breadcrumb-item>holding</n-breadcrumb-item>
    </n-breadcrumb>
    ...
    <div class="card-tnt">
      ...
      >
        <router-link
          :to="{ path: '/setting/master-holding/create/1' }"
      ...
      <template
        v-if="$filters.isGranted(constants.accessSettings.settingHolding + '.list')"
      >
        <n-row
          gutter="16"
