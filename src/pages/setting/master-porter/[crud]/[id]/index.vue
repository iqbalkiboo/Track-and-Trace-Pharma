...
onMounted(() => {
  if (route.params.crud == "create") {
    ...
    if (filters.isGranted(constants.accessSettings.settingPorter + ".create")) {
      getData()
    } else {
    ...
    if (filters.isGranted(constants.accessSettings.settingPorter + ".update")) {
      getData()
    } else {
      permissionModal.value.show = true
    }
...
rules.value = {
  name: {
    required: true,
    message: "Nama porter tidak boleh kosong",
    trigger: ["input", "blur"],
  },
  username: {
    required: true,
...
function showConfirmFeedback() {
  ...
  if (crudType.value == "create") {
    tFeedbackRef.value.subtitle =
      "Apakah Anda yakin untuk menambah data porter?"
  } else {
    tFeedbackRef.value.subtitle =
      "Apakah Anda yakin untuk mengubah data porter?"
  }
...
function showSuccessFeedback() {
  ...
  if (crudType.value == "create") {
    ...
    tFeedbackRef.value.subtitle = "Data porter berhasil ditambahkan"
  } else {
    tFeedbackRef.value.title = "Berhasil Ubah Data"
    tFeedbackRef.value.subtitle = "Data porter berhasil diubah"
  }
  tFeedbackRef.value.show = true
}
...
<template>
  <div class="body-tnt">
    <n-h2 style="margin-bottom: 4px">
      {{ crudType == "create" ? "Tambah Porter" : "Edit Porter" }}
    </n-h2>
    <n-breadcrumb>
      <n-breadcrumb-item>
        <router-link
        ...
        >
          ...
          >
            <n-input
              ...
              placeholder="Masukkan Nama Porter"
            />
          </n-form-item>
          <n-form-item
            path="username"
          ...
          <n-form-item
            v-show="false"
            path="roleUser"
            :label="'Pilih ' + accessValue.label"
          >
            <n-select
              v-model:value="formValue.roleUser"
              :options="roleUserOptions"
              :placeholder="'Pilih ' + accessValue.label"
            />
          </n-form-item>
          <n-form-item
            path="phone"
          ...
          >
            <n-select
              ...
              placeholder="Pilih Status Porter"
              style="width: 100%"
            />
          </n-form-item>
        </n-form>
    ...
    <div class="bottom-action-button">
      <div class="action-item">
        ...
        <template v-if="crudType === 'create' && $filters.isGranted(constants.accessSettings.settingPorter + '.create')">
          <n-button
            color="#219653"
            class="button-tnt"
        ...
        <template v-if="crudType === 'update' && $filters.isGranted(constants.accessSettings.settingPorter + '.update')">
          <n-button
            color="#219653"
            class="button-tnt"
            :loading="isBtnLoading"
