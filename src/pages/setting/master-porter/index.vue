...
onMounted(async () => {
  setIsGranted()
  if (filters.isGranted(constants.accessSettings.settingPorter + ".list")) {
    await getData()
    getWarehouseType()
  } else {
    return router.push('/')
...
function setIsGranted() {
  ...
  if (filters.isGranted(constants.accessSettings.settingPorter + ".delete")) {
    actionOptions.value.push({
      label: "Hapus Data",
      key: "delete",
...
function handleSelectDropdown(key: string | number, id: any, name: any) {
  switch (key) {
    case "edit":
      router.push({
        path: `/setting/master-porter/update/${id}`,
      })
      break
    case "delete":
...
function showSuccessFeedback() {
  ...
  tFeedbackRef.value.subtitle = "Anda berhasil menghapus data porter"
...
<template>
  <div class="body-tnt">
    <n-h2 style="margin-bottom: 4px">Master Porter</n-h2>
    <n-breadcrumb>
      <n-breadcrumb-item>Master Porter</n-breadcrumb-item>
    </n-breadcrumb>

    <div class="card-tnt">
      <n-space
        v-if="$filters.isGranted(constants.accessSettings.settingPorter + '.create')"
      >
        <router-link
          :to="'/setting/master-porter/create/1'"
      ...
      <template
        v-if="$filters.isGranted(constants.accessSettings.settingPorter + '.list')"
      >
        <n-row
          gutter="16"
