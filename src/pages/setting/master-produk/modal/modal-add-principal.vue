  ...
  name: [
    {
      ...
      validator: (rule: FormItemRule, value: string | undefined) => {
        if (!value || value.length < 1)
          return new Error('Nama Principal tidak boleh kosong')
      },
      trigger: ['input', 'blur'],
    },
  ],
...
function handleAddPrincipal() {
  ...
  principalModel.createPrincipal(formData).then(() => {
  ...
  <n-modal
    ...
    >
      <n-space vertical :size="25">
        <n-h3 style="margin-bottom: 0">Tambah Principal</n-h3>
        <n-form ref="formRef" class="form-tnt" :model="formValue" :rules="rules" :show-require-mark="false">
          <n-form-item path="name" label="Nama Principal">
            <n-input
              v-model:value="formValue.name"
              placeholder="Masukkan Nama Principal"
            />
          </n-form-item>
          <n-form-item path="erp_code" label="Kode Principal">
            <n-input
        ...
        <n-space justify="center">
          <n-button color="#828282" class="button-tnt" :loading="isBtnLoading" @click="hideModal"
            >Batalkan</n-button
          >
          <n-button color="#219653" class="button-tnt" :loading="isBtnLoading" @click="handleAddPrincipal"
            >Simpan</n-button
          >
        </n-space>
      </n-space>
