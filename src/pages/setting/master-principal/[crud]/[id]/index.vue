...
rules.value = {
  name: {
    required: true,
    message: "Nama Principal tidak boleh kosong",
    trigger: "blur",
  },
  description: {
    required: true,
  ...
  erp_code: {
    required: true,
    message: "Kode Principal tidak boleh kosong",
    trigger: "blur",
  },
}
...
onMounted(() => {
  ...
  if (route.params.crud == "create") {
    ...
    if (filters.isGranted(constants.accessSettings.settingPrincipal + '.create')) {
      getData()
    } else {
...
function showConfirmFeedback() {
  ...
  if (crudType.value == "create") {
    tFeedbackRef.value.subtitle = "Apakah Anda yakin untuk menambah principal?"
  } else {
    tFeedbackRef.value.subtitle = "Apakah Anda yakin untuk mengubah principal?"
  }
...
function showSuccessFeedback() {
  ...
  if (crudType.value == "create") {
    ...
    tFeedbackRef.value.subtitle = "Data principal berhasil ditambahkan"
  } else {
    tFeedbackRef.value.title = "Berhasil Ubah Data"
    tFeedbackRef.value.subtitle = "Data principal berhasil diubah"
  }
  tFeedbackRef.value.show = true
}
</script>
  ...
  <div class="body-tnt">
    ...
    <div class="card-tnt">
      ...
      <template v-else>
        <n-form
          ...
          :rules="rules"
        >
          <n-form-item label="Nama Principal" path="name">
            <n-input
              v-model:value="formValue.name"
              placeholder="Masukkan Nama Principal"
            />
          </n-form-item>
          <n-form-item label="Kode Principal" path="erp_code">
            <n-input
              v-model:value="formValue.erp_code"
              placeholder="Masukkan Kode Principal"
            />
          </n-form-item>
          <n-form-item label="Deskripsi" path="description">
            <n-input
              v-model:value="formValue.description"
              placeholder="Ketikkan Deskripsi Principal"
              type="textarea"
    ...
    <div class="bottom-action-button">
      <div class="action-item">
        ...
        <template v-if="crudType === 'create' && $filters.isGranted(constants.accessSettings.settingPrincipal + '.create')">
          <n-button
            color="#219653"
            class="button-tnt"
        ...
        <template v-if="crudType === 'update' && $filters.isGranted(constants.accessSettings.settingPrincipal + '.create')">
          <n-button
            color="#219653"
            class="button-tnt"
            :loading="isBtnLoading"
  ...
  >
    <n-space justify="center">
      ...
      <template v-if="tFeedbackRef.type == 'success'">
        ...
        <router-link
          :to="{ name: 'setting-master-principal' }"
          class="text-decoration-none"
        >
