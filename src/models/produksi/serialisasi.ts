...
export const serializeModel = {
  ...
  async softDeleteSerialize(id: string, params: FormData) {
    const res = await api.delete(`/penerimaan/v1/soft-delete/${id}`, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
      data: params,
  ...
  async getGenerateSerialize(id: string, qty: number, lot: string, highest_retail_price?: number) {
    const params: any = {
      company_id: getCompanyId,
      qty,
      lot,
  ...
  async postReportDriverPrinter(params: any, type: string) {
    // params.append("company_id", getCompanyId)
